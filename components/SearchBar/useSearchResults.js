...
const useSearchResults = () => {
  ...
  const searchMapAddresses = useCallback(
    async (term) => {
      ...
      const geocodingClient = Geocoding({accessToken: process.env.NEXT_PUBLIC_MAPBOX_KEY});
      const mapboxResp = await geocodingClient.forwardGeocode({
        query: parsed.address,
        limit: 2,
        countries: parsed.countries,
      }).send();

      const mapAddressResultsPromise = Promise.all(mapboxResp.body.features.map(async (feature) => {
        ...
        const hexagons = geojson2h3.featureToH3Set(tinyPolygon, 8, {ensureOutput: true});
        ...
        const hotspots = await (await client.hotspots.hex(index)).take(20);
      ...
      dispatch({ type: PUSH_RESULTS, payload: { results: mapAddressResults, term } })
    },
    [dispatch],
  )
